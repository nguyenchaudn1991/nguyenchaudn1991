...
- Trang tĩnh greenfield độc lập (report/content/LP): mặc định **0 framework, 0 bundler** —
  1 file HTML + CSS + (nếu cần) 1 file JS nhỏ `defer`. Trong repo đã có framework/bundler,
  giữ stack hiện hữu và tối ưu trong stack đó; không dựng một hệ thứ hai chỉ để đạt luật này.
