...
> **Dependencies & fallback — check availability BEFORE starting.** This skill is not
> self-contained. It needs:
> - **A PPTX writer** — `pptxgenjs`, or any equivalent OOXML pack/unpack path. If none is
>   available, **stop and tell the user what is missing.** Do not emit a substitute format
>   (Markdown, HTML, PDF) and present it as the deliverable.
> - **A pptx→text extractor** for the §5.2 content QA — `python -m markitdown`, or any
>   equivalent. If none is available, do that QA by re-reading the generation source
>   instead, and state in the handoff that text-extraction QA was skipped.
> - **A renderer** for the §5.3 visual QA. If none is available, say so explicitly rather
>   than claiming the deck was visually verified.
>
> The user's header/footer template is applied by the user afterward and is **not** needed
> by the agent — just respect the safe band in §1b.
...
detail**. The agent may write the full **explanation** here — how something works, why it
