...
Bundle bằng esbuild (Lambda Node 20/22):
```bash
esbuild src/lambda.ts --bundle --platform=node --target=node20 \
...
| Cách | Khi nào |
|---|---|
| **Lambda Function URL + CloudFront** | Mặc định cho web SSR — rẻ, đơn giản, CloudFront lo custom domain + cache + WAF |
...
// + CloudFront Distribution: default → FunctionUrlOrigin(url), /assets/* → S3Origin(bucket)
```
