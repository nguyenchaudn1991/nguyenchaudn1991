...
> **Phụ thuộc & fallback:** `jp-comm` / `meo-pptx` / `premium-web` là **optional** —
> chỉ dùng khi runtime có sẵn. Không có → tự làm phần đó theo reference của skill này
> (bảng Q&A tự dựng cột 番号/質問/背景/回答期限/ステータス), ghi rõ trong output là đã
> làm thủ công. **Không giả định skill kia tồn tại và không dừng việc vì thiếu nó.**
> Đọc file nguồn (.pptx/.md/.html/Figma) bằng công cụ sẵn có của runtime; không có tool
> đọc được định dạng đó → yêu cầu người dùng export text/PDF/ảnh. **Không đoán nội dung
> file mình chưa đọc được, và không nói là đã đọc.**
